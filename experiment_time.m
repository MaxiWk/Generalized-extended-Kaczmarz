function data = experiment(n,m,sp,lambda_value,T,L_gstar,maxiter,time_per_repeat,num_repeats,iter_save,rowsamp,colsamp,seed_indices,writeout,disp_instance,savestep,method_array,experiment_description)
...
[res, adaptive_res, lsres, res_proj, lsres_proj, err_to_sparse, err_to_moorepi, nonzero_entries]...
...
[maxiter_list, last_res, last_lsres, last_adaptive_res, last_err_to_sparse, last_err_to_moorepi, last_nonzero_entries]...
                     = deal( zeros(num_repeats, num_methods) );
...
for repeats = 1:num_repeats 
    ...
    maxiter_list(repeats, method_counter) = iter;
    last_res(repeats, method_counter) = res(iter_save_counter, repeats, method_counter);
    last_lsres(repeats, method_counter) = lsres(iter_save_counter, repeats, method_counter);
    last_adaptive_res(repeats, method_counter) = adaptive_res(iter_save_counter, repeats, method_counter);
    last_err_to_moorepi(repeats, method_counter) = err_to_moorepi(iter_save_counter, repeats, method_counter);
    last_err_to_sparse(repeats, method_counter) = err_to_sparse(iter_save_counter, repeats, method_counter);
    last_nonzero_entries(repeats, method_counter) = nonzero_entries(iter_save_counter, repeats, method_counter);
...
for i = 1:length(method_array)
      disp(['Adaptive res of last iterate, ' method_array{i} ' method (min, median, max): ' num2str(min_last_adaptive_res(i)) ', ' num2str(median_last_adaptive_res(i)) ', ' num2str(max_last_adaptive_res(i)) ])
end

for i = 1:length(method_array)
    disp(['Least squares res of last iterate, ' method_array{i} ' method (min, median, max): ' num2str(min_last_lsres(i)) ', ' num2str(median_last_lsres(i)) ', ' num2str(max_last_lsres(i)) ])
end

for i = 1:length(method_array)
    disp(['Error to sparse vector, ' method_array{i} ' method (min, median, max): ' num2str(min_last_err_to_sparse(i)) ', ' num2str(median_last_err_to_sparse(i)) ', ' num2str(max_last_err_to_sparse(i)) ])
end

for i = 1:length(method_array)
    disp(['Entries with absolute value > ' num2str(tol_zero) ', ' method_array{i} ' method (min, median, max): ' num2str(min_last_nonzero_entries(i)) ', ' num2str(median_last_nonzero_entries(i)) ', ' num2str(max_last_nonzero_entries(i)) ])
end



% take min/max/median of maxiter / interesting quantities for last iterates
min_maxiter_list = min(maxiter_list,[],1);
max_maxiter_list = max(maxiter_list,[],1);
median_maxiter_list = median(maxiter_list,1);

min_last_adaptive_res = min(last_adaptive_res,[],1);
max_last_adaptive_res = max(last_adaptive_res,[],1);
median_last_adaptive_res = median(last_adaptive_res,[],1);

min_last_lsres = min(last_lsres,[],1);
max_last_lsres = max(last_lsres,[],1);
median_last_lsres = median(last_lsres,[],1);

min_last_err_to_moorepi = min(last_err_to_moorepi,[],1);
max_last_err_to_moorepi = max(last_err_to_moorepi,[],1);
median_last_err_to_moorepi = median(last_err_to_moorepi,[],1);

min_last_err_to_sparse = min(last_err_to_sparse,[],1);
max_last_err_to_sparse = max(last_err_to_sparse,[],1);
median_last_err_to_sparse = median(last_err_to_sparse,[],1);

min_last_nonzero_entries = min(last_nonzero_entries,[],1);
max_last_nonzero_entries = max(last_nonzero_entries,[],1);
median_last_nonzero_entries = median(last_nonzero_entries,[],1);
...
data = struct('method_array', method_array, 'xhat_list', xhat_list, 'yhat_list', yhat_list, ...
              ...
              'min_maxiter_list', min_maxiter_list, 'median_maxiter_list', median_maxiter_list, 'max_maxiter_list', max_maxiter_list, ...
              'min_last_adaptive_res', min_last_adaptive_res, 'median_last_adaptive_res', median_last_adaptive_res,'max_last_adaptive_res',max_last_adaptive_res, ...
              'min_last_lsres', min_last_lsres, 'median_last_lsres', median_last_lsres,'max_last_lsres',max_last_lsres, ...
              'min_last_err_to_moorepi', min_last_err_to_moorepi, 'max_last_err_to_moorepi',max_last_err_to_moorepi,'median_last_err_to_moorepi',median_last_err_to_moorepi,...
              'min_last_err_to_sparse', min_last_err_to_sparse, 'max_last_err_to_sparse',max_last_err_to_sparse,'median_last_err_to_sparse',median_last_err_to_sparse,...
              'min_last_nonzero_entries',min_last_nonzero_entries, 'max_last_nonzero_entries', max_last_nonzero_entries, 'median_last_nonzero_entries', median_last_nonzero_entries
              );
