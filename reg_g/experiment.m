...
fprintf('Experiment A, n=%d, m=%d, sp=%d, maxiter=%d, num_repeats=%d,\n',n,m,sp,maxiter,num_repeats)
...
for repeats = 1:num_repeats 
    ...
    for method_counter = 1:length(method_array)
        ...
        % Initialize method
        x = zeros(n,1);  % Initialization for sparse randomized kaczmarz
        xdual = zeros(n,1);  
        zdual = b;
        z = T(zdual);
        
        % precompute expressions for det_sek (only for small dimensions)
        AAT = A*A'; 
        det_sek_stepsize = 1/norm(A)^2;
      ...
      for iter = 1:maxiter
          ...
          switch method
            ...
            case 'det_sek'
              zdual = zdual - det_sek_stepsize*AAT*zdual;
              z = T(zdual);
              xdual = xdual - det_sek_stepsize*A'*(A*x-b+zdual);
              x = S(xdual);
...
minmaxcolor_dict('det_sek') = lightgray;
...
quantcolor_dict('det_sek') = mediumgray;
...
medianplot_array = plot_minmax_median_quantiles(min_res,max_res,median_res,quant25_res,quant75_res);

hold off

title([sprintf('Residuals; n = %d, m = %d, s = %d, repeats = %d, sampling: ',n,m,sp,num_repeats) rowsamp])
...
medianplot_array = plot_minmax_median_quantiles(min_res_proj,max_res_proj,median_res_proj,quant25_res_proj,quant75_res_proj);

hold off

title([sprintf('Proj. Residuals; n = %d, m = %d, s = %d, repeats = %d, sampling: ',n,m,sp,num_repeats) rowsamp])
%}
...
medianplot_array = plot_minmax_median_quantiles(min_err_to_sparse,max_err_to_sparse,median_err_to_sparse,...
                                                quant25_err_to_sparse,quant75_err_to_sparse);
                                                
hold off                                                

title([sprintf('Distance to sparse solution (RegBP); n = %d, m = %d, s = %d, repeats = %d, sampling: ',n,m,sp,num_repeats) rowsamp])
...
medianplot_array = plot_minmax_median_quantiles(min_err_to_moorepi,max_err_to_moorepi,median_err_to_moorepi,...
                                                quant25_err_to_moorepi,quant75_err_to_moorepi);
                                                
hold off                                                

title([sprintf('Distance to Moore Penrose solution; n = %d, m = %d, s = %d, repeats = %d, sampling: ',n,m,sp,num_repeats) rowsamp])
...
medianplot_array = plot_minmax_median_quantiles(min_lsres,max_lsres,median_lsres,quant25_lsres,quant75_lsres);

hold off

title([sprintf('Gradient of least squares function; n = %d, m = %d, s = %d, repeats = %d, sampling: ',n,m,sp,num_repeats) rowsamp])
...
function medianplot_array = plot_minmax_median_quantiles(mins,maxs,medians,quant25s,quant75s)
