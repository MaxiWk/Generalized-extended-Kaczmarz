function data = experiment(n,m,sp,real_setting,lambda_value,T,L_gstar,maxiter,num_repeats,iter_save,rowsamp,colsamp,seed_indices,writeout,disp_instance,savestep,method_array,experiment_description)
...
for repeats = 1:num_repeats 
    ...
    [A,b,b_exact,xhat] = set_up_instance(m,n,sp,real_setting,experiment_description);
    rankA = rank(A);
...
%% Plot entries of xhat and the last iterate x_k
...
if real_setting
    ...
    %subplot(2,1,2); plot(1:n,x_hat,1:n,x_stop,':'); title(['x_{hat}, x, nnz_{hat}=',num2str(nnz_hat),' , nnz=',num2str(numel(find(x_stop)))]);
else
    figure
    plot(1:m,abs(b_exact),1:m,abs(b),':'); title('|b_{hat}|, |b|');
    figure
    stem([abs(xhat),abs(x)]); title(['|x_{hat}| , nnz_{hat}=', num2str(sp),' , |x| , nnz=',num2str(length(find(abs(x)>1e-5))),' , M-Mult=',num2str(round(iter/max(m,n)))]);
    %subplot(2,1,2); plot(1:n,abs(x_hat),1:n,abs(x_stop),':'); title(['x_{hat}, x, nnz_{hat}=',num2str(nnz_hat),' , nnz=',num2str(numel(find(x_stop)))]);
end
